...
int historyBufferScroll(int n) {
  ...
  int p = abs(n = (n < 0) ? max(n, -term.row) : min(n, term.row)),
      r = term.row - p, s = sizeof(*term.dirty),
...
void selnormalize(void) {
  ...
  if (sel.ob.x == -1) {
    ...
    int const nby = rangeY(sel.ob.y - off), ney = rangeY(sel.oe.y - off);
    sel.swap =
        rangeY(sel.ob.y - offsetBuffer) > rangeY(sel.oe.y - offsetBuffer);
  ...
  for (int i = 0; i < term.row; ++i) {
    int const n =
        nBet ? BETWEEN(i, sel.nb.y, sel.ne.y) : OUT(i, sel.nb.y, sel.ne.y);
    term.dirty[i] |=
        (sel.type == SEL_RECTANGULAR && n) ||
...
int selected(int x, int y) {
  ...
  return ((sel.nb.y > sel.ne.y) ? OUT(y, sel.nb.y, sel.ne.y)
         ...
         (y != sel.nb.y || x >= sel.nb.x) && (y != sel.ne.y || x <= sel.ne.x);
#else
...
void csihandle(void) {
    ...
    case 2: /* screen */
      ...
      if (!IS_SET(MODE_ALTSCREEN)) {
        ...
        for (n = term.row - 1; n >= 0; n--) {
          for (m = 0;
               m < maxcol && term.line[n][m].u == ' ' && !term.line[n][m].mode;
               m++)
            ;
          if (m < maxcol) {
#if SCROLLBACK_PATCH
  ...
  case 'n': /* DSR – Device Status Report (cursor position) */
    if (csiescseq.arg[0] == 6) {
      len = snprintf(buffer, sizeof(buffer), "\033[%i;%iR", term.c.y + 1,
                     term.c.x + 1);
    ...
    if (IS_SET(MODE_SIXEL)) {
      ...
      for (i = 0; i < (sixel_st.image.height + win.ch - 1) / win.ch; ++i) {
        ...
        tclearregion(term.c.x, term.c.y,
                     term.c.x + (sixel_st.image.width + win.cw - 1) / win.cw,
                     term.c.y);
        for (x = term.c.x;
             x < MIN(term.col,
                     term.c.x + (sixel_st.image.width + win.cw - 1) / win.cw);
...
/* resize to new height */
...
#if SCROLLBACK_PATCH
  Glyph gc =
      (Glyph){.bg = term.c.attr.bg, .fg = term.c.attr.fg, .u = ' ', .mode = 0};
  ...
  term.line =
      &buf[*(histOp ? &histOff : &insertOff) += MAX(term.c.y - row + 1, 0)];
