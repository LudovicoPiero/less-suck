...
#define SIXEL_XRGB(r, g, b)                                                    \
  SIXEL_RGB(SIXEL_PALVAL(r, 255, 100), SIXEL_PALVAL(g, 255, 100),              \
...
static int sixel_image_init(sixel_image_t *image, int width, int height,
...
int sixel_parser_init(sixel_state_t *st, sixel_color_t fgcolor,
  ...
  while (p < p0 + len) {
    ...
    case PS_DECGCI:
      ...
      default:
        ...
        if (st->nparams > 0) {
          st->color_index =
              1 + st->params[0]; /* offset 1(background color) added */
          if (st->color_index < 0)
            st->color_index = 0;
