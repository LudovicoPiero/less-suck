...
int width_pwrl_status_es(Bar *bar, BarArg *a) {
#if BAR_STATUSCMD_PATCH
...
#if BAR_EXTRASTATUS_PATCH
int draw_pwrl_status_es(Bar *bar, BarArg *a) {
#if BAR_STATUSCMD_PATCH
...
int click_pwrl_status(Bar *bar, Arg *arg, BarArg *a) { return ClkStatusText; }
...
int drawpowerlinestatus(int xpos, char *stext, BarArg *barg) {
  ...
  for (i = n, bs = &status[n - 1]; i >= 0; i--, bs--) {
    if (*bs == '<' || *bs == '/' || *bs == '\\' || *bs == '>' ||
      ...
      if (bp != '|') {
        drw_arrow(drw, x - plw, barg->y, plw, barg->h,
                  bp == '\\' || bp == '>' ? 1 : 0, bp == '<' ? 0 : 1);
        x -= plw;
      }
