void persistmonitorstate(Monitor *m) {
  Client *c;
  unsigned int i;
...
int restoremonitorstate(Monitor *m) {
  return getmonitortags(m) | getmonitorfields(m);
}

void persistclientstate(Client *c) {
  setclienttags(c);
  setclientfields(c);
#if SAVEFLOATS_PATCH
...
int restoreclientstate(Client *c) {
  return getclienttags(c) | getclientfields(c)
#if SAVEFLOATS_PATCH
 ...
 */
  ...
  for (i = 0; i <= NUMTAGS; i++) {
    ...
    XChangeProperty(dpy, root, monitor_fields, XA_CARDINAL, 32,
                    i ? PropModeAppend : PropModeReplace, (unsigned char *)data,
                    1);
  }
#else // !PERTAG_PATCH
#if FLEXTILE_DELUXE_LAYOUT
  ...
  if (m->lt[m->sellt]->arrange == flextile) {
    flextile_deluxe_bitmask |=
        (abs(m->ltaxis[LAYOUT]) & 0xF) << 10 | (m->ltaxis[MASTER] & 0xF) << 14 |
        (m->ltaxis[STACK] & 0xF) << 18 | (m->ltaxis[STACK2] & 0xF) << 22 |
  ...
  uint32_t data[] = {
    ...
    flextile_deluxe_bitmask |
#endif // FLEXTILE_DELUXE_LAYOUT
    (m->nmaster & 0x7) | (getlayoutindex(m->lt[m->sellt]) & 0xF) << 6 |
    m->showbar << 31
  };
...
int getlayoutindex(const Layout *layout) {
  int i;
...
int getmonitorfields(Monitor *m) {
  int di, layout_index;
#if PERTAG_PATCH
...
#if PERTAG_PATCH
  for (i = 0; i <= NUMTAGS; i++) {
    if (!(XGetWindowProperty(dpy, root, dwm_monitor, i,
                             (NUMTAGS + 1) * sizeof dl, False, AnyPropertyType,
  ...
  return restored;
#else // !PERTAG_PATCH
  if (!(XGetWindowProperty(dpy, root, dwm_monitor, 0L, sizeof dl, False,
                           AnyPropertyType, &da, &di, &nitems, &dl,
                           &p) == Success &&
        p)) {
    return 0;
  }
...
void setmonitortags(Monitor *m) {
  ...
  XChangeProperty(dpy, root, monitor_tags, XA_CARDINAL, 32, PropModeReplace,
                  (unsigned char *)data, 1);
}

int getmonitortags(Monitor *m) {
  int di;
  unsigned long dl, nitems;
  unsigned char *p = NULL;
  char atom[22] = {0};
  ...
  if (!(XGetWindowProperty(dpy, root, monitor_tags, 0L, sizeof dl, False,
                           AnyPropertyType, &da, &di, &nitems, &dl,
                           &p) == Success &&
        p)) {
    return 0;
...
void setclientfields(Client *c) {
   ...
   */
  uint32_t data[] = {
    (c->mon->num & 0x7) | (c->idx & 0xFF) << 3 | (c->isfloating & 0x1) << 11
#if ISPERMANENT_PATCH
    | (c->ispermanent & 0x1) << 12
#endif // ISPERMANENT_PATCH
#if SWALLOW_PATCH
    | (c->isterminal & 0x1) << 13 | (c->noswallow & 0x1) << 14
#endif // SWALLOW_PATCH
#if STEAM_PATCH
    | (c->issteam & 0x1) << 15
#endif // STEAM_PATCH
  ...
  XChangeProperty(dpy, c->win, clientatom[ClientFields], XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)data, 1);
}

int getclientfields(Client *c) {
  Monitor *m;
...
void setclienttags(Client *c) {
  uint32_t data[] = {c->tags};
  XChangeProperty(dpy, c->win, clientatom[ClientTags], XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)data, 1);
}

int getclienttags(Client *c) {
...
#if SAVEFLOATS_PATCH
void savewindowfloatposition(Client *c, Monitor *m) {
  char atom[22] = {0};
  if (c->sfx == -9999)
    return;

  sprintf(atom, "_DWM_FLOATPOS_%u", m->num);
  uint32_t pos[] = {(MAX(c->sfx - m->mx, 0) & 0xffff) |
                    ((MAX(c->sfy - m->my, 0) & 0xffff) << 16)};
  XChangeProperty(dpy, c->win, XInternAtom(dpy, atom, False), XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)pos, 1);
...
int restorewindowfloatposition(Client *c, Monitor *m) {
  char atom[22] = {0};
  Atom key, value;
  int x, y, w, h;
  ...
  if (w <= 0 || h <= 0) {
    fprintf(stderr,
            "restorewindowfloatposition: bad float values x = %d, y = %d, w = "
            "%d, h = %d for client = %s\n",
            x, y, w, h, c->name);
    return 0;
  }
