int width_taggrid(Bar *bar, BarArg *a) {
  return (a->h / 2) * (NUMTAGS / tagrows + ((NUMTAGS % tagrows > 0) ? 1 : 0)) +
         lrpad;
}

int draw_taggrid(Bar *bar, BarArg *a) {
    ...
    for (k = 0; k < columns; k++, i++) {
      if (i < NUMTAGS) {
        ...
        XSetForeground(drw->dpy, drw->gc,
                       !invert ? scheme[SchemeTagsSel][ColBg].pixel
        ...
        if (occ & 1 << i) {
          XSetForeground(drw->dpy, drw->gc,
                         !invert ? scheme[SchemeTagsSel][ColFg].pixel
                                 : scheme[SchemeTagsNorm][ColBg].pixel);
          XFillRectangle(dpy, drw->drawable, drw->gc, x + 1, y + 1, h / 2,
                         h / 2);
        }
      } else {
        XSetForeground(drw->dpy, drw->gc, scheme[SchemeTagsNorm][ColBg].pixel);
...
int click_taggrid(Bar *bar, Arg *arg, BarArg *a) {
