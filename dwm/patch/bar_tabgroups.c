...
#endif
                           ? SchemeSel
                           : groupactive ? SchemeTitleSel : SchemeTitleNorm]);
  if (w <=
...
void bartabclick(Monitor *m, Client *c, int passx, int x, int w, int unused,
                 Arg *arg, BarArg *barg) {
  if (passx >= x && passx <= x + w)
    arg->v = c;
}

int bartabcalculate(Monitor *m, int offx, int tabw, int passx,
                    void (*tabfn)(Monitor *, Client *, int, int, int, int,
                                  Arg *arg, BarArg *barg),
                    Arg *arg, BarArg *barg) {
  Client *c;
  int i, clientsnmaster = 0, clientsnstack = 0, clientsnfloating = 0,
         clientsnhidden = 0, masteractive = 0, fulllayout = 0, x = offx, w, r,
         num = 0, den, tgactive;
  ...
  num = tabw;
  /* floating mode */
  if ((fulllayout && BARTAB_FLOATWEIGHT) ||
    ...
    i = 0;
    if (den) {
      if (clientsnstack + clientsnfloating * BARTAB_FLOATWEIGHT +
          clientsnhidden) {
    ...
    den = clientsnstack * BARTAB_STACKWEIGHT +
          clientsnfloating * BARTAB_FLOATWEIGHT +
          clientsnhidden * BARTAB_HIDDENWEIGHT;
    if (!den)
      return 1;
    ...
    for (; c; c = c->next) { // tiled stack
      if (!ISVISIBLE(c) || HIDDEN(c) || c->isfloating)
        continue;
      tabfn(m, c, passx, x,
            w * BARTAB_STACKWEIGHT + (i - m->nmaster < r ? 1 : 0), tgactive,
            arg, barg);
    ...
    for (c = m->clients; c; c = c->next) { // hidden windows
      if (!ISVISIBLE(c) || !HIDDEN(c))
        continue;
      tabfn(m, c, passx, x,
            w * BARTAB_HIDDENWEIGHT + (i - m->nmaster < r ? 1 : 0), tgactive,
            arg, barg);
    ...
    for (c = m->clients; c; c = c->next) { // floating windows
      if (!ISVISIBLE(c) || HIDDEN(c) || !c->isfloating)
        continue;
      tabfn(m, c, passx, x,
            w * BARTAB_FLOATWEIGHT + (i - m->nmaster < r ? 1 : 0), tgactive,
            arg, barg);
