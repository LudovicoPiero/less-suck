int width_pwrl_tags(Bar *bar, BarArg *a) {
  int w, i;
...
int draw_pwrl_tags(Bar *bar, BarArg *a) {
  int x, w;
  int invert;
  int plw = drw->fonts->h / 2 + 1;
  ...
  for (i = 0; i < NUMTAGS; i++) {
    ...
    if (urg & 1 << i) {
      drw_settrans(
          drw, prevscheme,
          (nxtscheme = scheme[bar->mon->tagset[bar->mon->seltags] & 1 << i
                                  ? SchemeSel
                                  : SchemeUrg]));
    } else {
      drw_settrans(
    ...
    drawindicator(bar->mon, NULL, occ, x, a->y, w, a->h, i, -1, invert,
                  tagindicatortype);
    x += w;
...
int click_pwrl_tags(Bar *bar, Arg *arg, BarArg *a) {
  int i = 0, x = lrpad / 2;
  int plw = drw->fonts->h / 2 + 1;
#if BAR_HIDEVACANTTAGS_PATCH
  Client *c;
