...
#if ATTACHABOVE_PATCH || ATTACHASIDE_PATCH || ATTACHBOTTOM_PATCH ||            \
    SEAMLESS_RESTART_PATCH
  Client *at;
#endif // ATTACHABOVE_PATCH | ATTACHASIDE_PATCH | ATTACHBOTTOM_PATCH |
       // SEAMLESS_RESTART_PATCH

#if SEAMLESS_RESTART_PATCH
  if (c->idx >
      0) { /* then the client has a designated position in the client list */
